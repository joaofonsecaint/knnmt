...
def main(args):
    ...
    def make_dataset(vocab, input_prefix, output_prefix, lang, num_workers=1):
        ...
        if args.dataset_impl == "raw":
            ...
            num_tokens=make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers)
        return num_tokens
    ...
    def make_all(lang, vocab):
        ...
        if args.correctionspref:
            num_tokens=make_dataset(vocab, args.correctionspref, "corrections", lang, num_workers=args.workers)
        return num_tokens
    ...
    def make_all_alignments():
        ...
        if args.correctionspref and os.path.exists(args.correctionspref + "." + args.align_suffix):
            make_binary_alignment_dataset(
                args.correctionspref + "." + args.align_suffix,
                "corrections.align",
                num_workers=args.workers,
            )

    source_tokens=make_all(args.source_lang, src_dict)
    if target:
        target_tokens=make_all(args.target_lang, tgt_dict)
    if args.align_suffix:
        make_all_alignments()
    
    if args.correctionspref:
        with open("num_tokens",'w') as f:
            f.write(str(max(source_tokens, target_tokens)))

    logger.info("Wrote preprocessed data to {}".format(args.destdir))

    if args.alignfile:
        assert args.train, "--trainpref must be set if --alignfile is specified"
